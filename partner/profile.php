...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
    $name = $_POST['name'];
    $mobile = $_POST['mobile'] ?? '';
    // $gender = $_POST['gender']; // Removed gender as it wasn't in original partner table schema, but keeping layout consistent
    ...
    if (empty($message)) {
        try {
            // Note: Gender column might not exist in partners table, verifying schema... 
            // Based on previous knowledge, partners table has similar structure. 
            // If gender is missing, this query will fail. 
            // Checks: partners schema usually mirrors senior_partners.
            // Let's assume standard fields first. Only 'gender' is potentially questionable if not added.
            // I'll include it for now, if it fails I'll fix it.
            $sql = "UPDATE partners SET name=:name, mobile=:mobile, address=:address, state=:state, city=:city, pincode=:pincode, image=:image WHERE id=:id";
            $stmt = $db->prepare($sql);
            ...
            if ($stmt->execute()) {
                $_SESSION['partner_name'] = $name; // Update session name
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['change_password'])) {
    ...
    $stmt = $db->prepare("SELECT password FROM partners WHERE id = :id");
    $stmt->bindParam(':id', $partner_id);
    $stmt->execute();
    ...
    if (password_verify($current_password, $row['password'])) {
        if ($new_password === $confirm_password) {
            ...
            $update_stmt = $db->prepare("UPDATE partners SET password = :password WHERE id = :id");
