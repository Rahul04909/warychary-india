...
// Query to get Senior Partners and their calculated earnings
$sql = "SELECT 
            sp.id, 
            sp.name, 
            sp.email,
        ...
        LEFT JOIN (
            SELECT senior_partner_id, SUM(amount) as total_earnings 
            FROM senior_partner_earnings 
            GROUP BY senior_partner_id
        ) e ON sp.id = e.senior_partner_id
        LEFT JOIN (
            SELECT user_id, SUM(amount) as total_paid 
            FROM payout_history 
            WHERE user_type = 'senior_partner' AND status = 'processed'
            GROUP BY user_id
        ) ph ON sp.id = ph.user_id
        LEFT JOIN bank_details bd ON sp.id = bd.user_id AND bd.user_type = 'senior_partner'
        WHERE 1=1";

if ($search) {
    $sql .= " AND (sp.name LIKE :search OR sp.email LIKE :search)";
}

$sql .= " ORDER BY sp.id DESC";

$countSql = "SELECT COUNT(*) FROM senior_partners sp WHERE 1=1";
if ($search) {
    $countSql .= " AND (sp.name LIKE :search OR sp.email LIKE :search)";
}
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between card flex-sm-row border-0">
            <h4 class="mb-sm-0 font-size-18 px-3">Senior Partner Payouts</h4>
            <div class="page-title-right px-3">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="javascript: void(0);">Payouts</a></li>
                    <li class="breadcrumb-item active">Senior Partner Payouts</li>
                </ol>
            </div>
        </div>
    </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <form class="d-flex" method="GET">
                            <input type="text" name="search" class="form-control me-2" placeholder="Search by name, email..." value="<?php echo htmlspecialchars($search); ?>">
                            <button type="submit" class="btn btn-primary">Search</button>
                        </form>
                    </div>
                    <div class="col-md-6 text-md-end mt-3 mt-md-0">
                        <form action="export-payouts.php" method="POST" target="_blank" class="d-inline">
                            <input type="hidden" name="type" value="senior_partner">
                ...
                <div class="table-responsive">
                    <table class="table align-middle table-nowrap table-hover">
                        ...
                        <tbody>
                            <?php if (count($partners) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <p class="text-muted mb-0"><?php echo htmlspecialchars($p['email']); ?></p>
                                    </td>
                                    <td>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" class="text-center">No senior partners found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
        ...
        <div class="modal-content">
            ...
            <form action="process-payout.php" method="POST">
                <div class="modal-body">
                    <input type="hidden" name="user_type" value="senior_partner">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnBulkPayout.addEventListener('click', function() {
        ...
        modalSummary.innerHTML = `Initiating Bulk Payout for <strong>${ids.length} Senior Partner(s)</strong>`;
